	...
	export class EndPanel2 extends BasePanel {
		...
		public init():void{
			...
			for(var index:number = 1; index < 5 ; index++){
				...
				bitmapText.text = fanNum.toString();
				...
				var headbg:egret.DisplayObject = itemSpr.getChildByName("headKuang");
				roleHead = new RoleHeadImage(role);
				roleHead.height = roleHead.width = 65;
				roleHead.x = headbg.x + 10;
				roleHead.y = headbg.y + 10;
				itemSpr.addChildAt(roleHead,itemSpr.getChildIndex(headbg));
